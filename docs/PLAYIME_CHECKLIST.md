...
- **OOC is now a text convention, not just a flag** (item 2 refinement). A message wrapped in asterisks (`*then Miko bowed*`) auto-flags as an OOC/stage-direction note — same path as the `ooc: true` flag (which the Phase 5 UI toggle will set). The asterisks are stripped before the note is rendered as `(Out-of-character note for the character: then Miko bowed)`; inline emphasis inside a normal turn (`Miko *smiles* warmly`) is NOT an OOC marker (guard: the inner text must contain no `*`). Detection lives in `backend/src/chat.ts` (`isOocWrapped`/`unwrapOoc`). Verified live: `*then Miko bows politely*` → reply `*Miko gives a graceful little bow*` — the direction was enacted in-fiction, never acknowledged. Documented in PLAYIME_PROMPT_SPEC §3 (OOC handling).
